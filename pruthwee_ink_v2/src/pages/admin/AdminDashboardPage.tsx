...
const PLATFORM_STATS = [
  { label: 'Total Volunteers',   value: 12483,   delta: '+214 this week',  up: true,  color: '#CCFF00', icon: <Users size={18} />     },
  { label: 'Active Events',      value: 18,      delta: '+3 vs last week', up: true,  color: '#6EE07A', icon: <Calendar size={18} />  },
  { label: 'Hours Logged',       value: '5.1L',  delta: '+8,200 this week',up: true,  color: '#CCFF00', icon: <Clock size={18} />     },
...
const RECENT_USERS = [
  { id: 'u1',  name: 'Vikram Desai',    email: 'vikram@email.com', role: 'volunteer', tier: 'Diamond', tierColor: '#CCFF00', mq: 99,  joined: 'Mar 10, 2026', status: 'active',   flagged: false },
...
const ACTIVITY_FEED = [
  ...
  { icon: '🎖️', text: 'Vikram Desai reached Diamond tier — 1000+ hours',      time: '1h ago',   color: '#CCFF00', action: false },
  { icon: '📋', text: '312 certificates issued today',                         time: '2h ago',   color: '#CCFF00', action: false },
  ...
  { icon: '📧', text: '1,240 emails dispatched — Sabarmati reminder',          time: '6h ago',   color: '#CCFF00', action: false },
];
...
const EVENT_STATUS_COLORS: Record<string, { color: string; bg: string; border: string }> = {
  ...
  closed: { color: '#CCFF00', bg: 'rgba(204,255,0,0.08)', border: 'rgba(204,255,0,0.2)'  },
};
...
function UserRow({ user, onAction }: { user: typeof RECENT_USERS[0]; onAction: (action: string, user: typeof RECENT_USERS[0]) => void }) {
  ...
  return (
    <div className={`flex items-center gap-4 px-5 py-3.5 border-b border-[rgba(204,255,0,0.06)] last:border-0 hover:bg-[rgba(204,255,0,0.03)] transition-colors ${user.flagged ? 'bg-[rgba(252,211,77,0.03)]' : ''}`}>
      <div className="w-9 h-9 rounded-full flex items-center justify-center flex-shrink-0 font-display font-black text-sm text-[#0C0C0C]"
        style={{ background: user.tierColor }}>
        {user.name.slice(0, 1)}
      </div>
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2">
          <p className="heading-gradient font-display font-black text-[#F2F2F2] text-sm uppercase tracking-wide truncate">{user.name}</p>
          {user.flagged && <AlertCircle size={12} className="text-[#FCD34D] flex-shrink-0" />}
        </div>
        <p className="font-sans text-[#888888] text-xs truncate">{user.email}</p>
      </div>
      <div className="hidden sm:flex flex-col items-end gap-1 flex-shrink-0">
        ...
        <span className="font-mono text-[#CCFF00] text-[10px]">MQ {user.mq}</span>
      </div>
      ...
      <div className="flex gap-1 flex-shrink-0">
        <button onClick={() => onAction('view', user)}
          className="w-7 h-7 rounded-lg bg-[rgba(255,255,255,0.05)] flex items-center justify-center text-[#CCFF00] hover:bg-[rgba(204,255,0,0.18)] transition-all">
          <Eye size={12} />
        </button>
...
function EventRow({ ev, onAction }: { ev: typeof RECENT_EVENTS[0]; onAction: (action: string, ev: typeof RECENT_EVENTS[0]) => void }) {
  ...
  return (
    <div className={`flex items-center gap-4 px-5 py-3.5 border-b border-[rgba(204,255,0,0.06)] last:border-0 hover:bg-[rgba(204,255,0,0.03)] transition-colors ${ev.flagged ? 'bg-[rgba(252,211,77,0.03)]' : ''}`}>
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2">
          <p className="heading-gradient font-display font-black text-[#F2F2F2] text-sm uppercase tracking-wide truncate">{ev.title}</p>
          {ev.flagged && <AlertCircle size={12} className="text-[#FCD34D] flex-shrink-0" />}
        </div>
        <p className="font-sans text-[#888888] text-xs">{ev.org} · {ev.city} · {ev.date}</p>
      </div>
      <div className="hidden sm:flex flex-col items-end gap-1.5 flex-shrink-0 min-w-[80px]">
        <span className="font-sans text-[#888888] text-xs">{ev.regs}/{ev.max}</span>
        <div className="w-16 h-1 bg-[rgba(255,255,255,0.08)] rounded-full">
          <div className="h-full rounded-full bg-[#CCFF00]" style={{ width: `${fillPct}%` }} />
        </div>
      </div>
  ...
  return (
    <div className="bg-[#0C0C0C] min-h-screen">
        ...
        <div className="max-w-7xl mx-auto px-5 md:px-8 py-4 flex items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <span className="heading-gradient font-display font-black text-[#F2F2F2] text-lg uppercase leading-none">Control Centre</span>
            </div>
          </div>
          <div className="flex items-center gap-3">
            {flaggedCount > 0 && (
      ...
      <div className="max-w-7xl mx-auto px-5 md:px-8 py-8 space-y-6">
        ...
        {/* Tab nav */}
        <SR>
          <div className="flex gap-1 bg-[rgba(20,20,20,0.4)] border border-[rgba(255,255,255,0.07)] rounded-xl p-1 overflow-x-auto">
            {ADMIN_TABS.map(t => (
              <button key={t.id} onClick={() => setTab(t.id)}
                className={`flex items-center gap-2 flex-shrink-0 px-4 py-2.5 rounded-lg font-display font-bold text-xs uppercase tracking-wide transition-all ${
                  tab === t.id
                    ? 'bg-[rgba(255,255,255,0.08)] text-[#CCFF00]'
                    : 'text-[#888888] hover:text-[#CCFF00]'
                }`}>
                {t.icon}{t.label}
              </button>
            ))}
        ...
        {tab === 'overview' && (
          <div className="space-y-6">
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
                {PLATFORM_STATS.map((s, i) => (
                  <motion.div key={s.label}
                    ...
                    className="bg-[rgba(20,20,20,0.5)] border border-[rgba(255,255,255,0.07)] rounded-2xl p-4 hover:border-[rgba(255,255,255,0.12)] transition-colors">
                    ...
                    <div className="font-display font-bold text-[#888888] text-[9px] uppercase tracking-[2px] mb-1">{s.label}</div>
                    <div className="font-sans text-[#CCFF00] text-[10px]">{s.delta}</div>
                  </motion.div>
                ))}
              </div>
            </SR>
            ...
            <div className="grid lg:grid-cols-[1fr_320px] gap-5">
              {/* Activity feed */}
              <SR delay={0.1}>
                <div className="bg-[rgba(20,20,20,0.4)] border border-[rgba(255,255,255,0.07)] rounded-2xl overflow-hidden">
                  <div className="flex items-center justify-between px-5 py-4 border-b border-[rgba(204,255,0,0.1)]">
                    <div className="flex items-center gap-2">
                      <Activity size={15} className="text-[#CCFF00]" />
                      <h3 className="heading-gradient font-display font-black text-[#F2F2F2] text-sm uppercase tracking-wide">Live Activity</h3>
                    </div>
                    <button className="text-[#CCFF00] hover:text-[#CCFF00] transition-colors">
                      <RefreshCw size={14} />
                    </button>
                  </div>
                  <div className="divide-y divide-[rgba(204,255,0,0.06)] max-h-96 overflow-y-auto">
                    {ACTIVITY_FEED.map((item, i) => (
                      <div key={i} className={`flex items-start gap-3 px-5 py-3 ${item.action ? 'bg-[rgba(252,211,77,0.03)]' : ''}`}>
                        <span className="text-base flex-shrink-0">{item.icon}</span>
                        <div className="flex-1 min-w-0">
                          <p className="font-sans text-[#888888] text-xs leading-snug">{item.text}</p>
                          <p className="font-mono text-[#CCFF00] text-[10px] mt-0.5">{item.time}</p>
                        </div>
                        {item.action && (
              ...
              <div className="space-y-4">
                    ...
                    <div className="divide-y divide-[rgba(252,211,77,0.07)]">
                      {[...RECENT_USERS.filter(u => u.flagged), ...RECENT_EVENTS.filter(e => e.flagged)].map((item: any, i) => (
                        <div key={i} className="flex items-center gap-3 px-5 py-3">
                          ...
                          <div className="flex-1 min-w-0">
                            <p className="font-display font-bold text-[#F2F2F2] text-xs uppercase tracking-wide truncate">
                              {item.name || item.title}
                            </p>
                            <p className="font-sans text-[#888888] text-[11px]">
                ...
                <SR delay={0.14}>
                  <div className="bg-[rgba(20,20,20,0.4)] border border-[rgba(255,255,255,0.07)] rounded-2xl p-4 space-y-2">
                    <p className="font-mono text-[#CCFF00] text-[10px] uppercase tracking-[2px] mb-3">Quick Actions</p>
                    {[
                      { label: 'Export All Volunteer Data', icon: <Download size={13} />, color: '#CCFF00'  },
                      ...
                      <button key={item.label} onClick={() => showToast(`${item.label}…`)}
                        className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl hover:bg-[rgba(255,255,255,0.04)] transition-colors group text-left">
                        <div className="w-6 h-6 rounded-lg flex items-center justify-center flex-shrink-0"
                          style={{ background: `${item.color}18`, color: item.color }}>
                          {item.icon}
                        </div>
                        <span className="font-display font-bold text-[#888888] text-xs uppercase tracking-wide group-hover:text-[#F2F2F2] transition-colors">
                          {item.label}
                        </span>
                      </button>
                    ))}
          ...
          <div className="space-y-5">
            <SR>
              <div className="flex flex-col sm:flex-row gap-3 items-start sm:items-center justify-between">
                <div>
                  <h2 className="heading-gradient font-display font-black text-[#F2F2F2] uppercase text-2xl tracking-wide">
                    User <span className="text-[#CCFF00]">Management</span>
                  </h2>
                  <p className="font-sans text-[#888888] text-sm">12,483 registered accounts</p>
                </div>
            ...
            <SR delay={0.05}>
              <div className="grid grid-cols-3 md:grid-cols-7 gap-2">
                {[
                  ...
                  { tier: 'Diamond',  count: 73,   color: '#CCFF00' },
                ].map(t => (
                  <div key={t.tier} className="bg-[rgba(20,20,20,0.5)] border border-[rgba(204,255,0,0.1)] rounded-xl p-3 text-center">
                    ...
                    <div className="font-display font-bold text-[#888888] text-[9px] uppercase tracking-[1.5px]">{t.tier}</div>
                  </div>
                ))}
              </div>
            </SR>
            ...
            <SR delay={0.08}>
              <div className="bg-[rgba(20,20,20,0.4)] border border-[rgba(255,255,255,0.07)] rounded-2xl overflow-hidden">
                <div className="px-5 py-3.5 border-b border-[rgba(204,255,0,0.1)]">
                  <h3 className="heading-gradient font-display font-black text-[#F2F2F2] text-sm uppercase tracking-wide">
                    Recent & Flagged Users
                  </h3>
                </div>
                <div className="divide-y divide-[rgba(204,255,0,0.06)]">
          ...
          <div className="space-y-5">
            <SR>
              <div className="flex flex-col sm:flex-row gap-3 items-start sm:items-center justify-between">
                <div>
                  <h2 className="heading-gradient font-display font-black text-[#F2F2F2] uppercase text-2xl tracking-wide">
                    Event <span className="text-[#CCFF00]">Moderation</span>
                  </h2>
                  <p className="font-sans text-[#888888] text-sm">320+ events total · 18 active</p>
                </div>
                <div className="relative w-full sm:w-64">
                  <Search size={13} className="absolute left-3 top-1/2 -translate-y-1/2 text-[#CCFF00]" />
            ...
            <SR delay={0.05}>
              <div className="bg-[rgba(20,20,20,0.4)] border border-[rgba(255,255,255,0.07)] rounded-2xl overflow-hidden">
                <div className="px-5 py-3.5 border-b border-[rgba(204,255,0,0.1)] flex items-center justify-between">
                  <h3 className="heading-gradient font-display font-black text-[#F2F2F2] text-sm uppercase tracking-wide">
                    Upcoming & Draft Events
                  </h3>
                  <span className="font-mono text-[#CCFF00] text-xs">{filteredEvents.length} shown</span>
                </div>
                <div>
        ...
        {tab === 'cities' && (
          <div className="space-y-5">
            <SR>
              <h2 className="heading-gradient font-display font-black text-[#F2F2F2] uppercase text-2xl tracking-wide">
                City <span className="text-[#CCFF00]">Breakdown</span>
              </h2>
            </SR>
            <SR delay={0.05}>
              <div className="bg-[rgba(20,20,20,0.4)] border border-[rgba(255,255,255,0.07)] rounded-2xl overflow-hidden">
                <div className="px-5 py-3.5 border-b border-[rgba(204,255,0,0.1)] grid grid-cols-4 gap-4">
                  {['City', 'Volunteers', 'Events', 'Hours'].map(h => (
                    <span key={h} className="font-mono text-[#CCFF00] text-[10px] uppercase tracking-[2px]">{h}</span>
                  ))}
                </div>
                {CITY_BREAKDOWN.map((row, i) => (
                  <motion.div key={row.city}
                    ...
                    transition={{ delay: i * 0.05 }}
                    className="grid grid-cols-4 gap-4 px-5 py-4 border-b border-[rgba(204,255,0,0.06)] last:border-0 hover:bg-[rgba(204,255,0,0.03)] transition-colors">
                    <div className="flex items-center gap-2">
                      <MapPin size={12} className="text-[#CCFF00]" />
                      <span className="heading-gradient font-display font-black text-[#F2F2F2] text-sm uppercase tracking-wide">{row.city}</span>
                    </div>
                    <span className="font-display font-bold text-[#CCFF00] text-sm">{row.volunteers.toLocaleString()}</span>
        ...
        {tab === 'system' && (
          <div className="space-y-5">
            <SR>
              <h2 className="heading-gradient font-display font-black text-[#F2F2F2] uppercase text-2xl tracking-wide">
                System <span className="text-[#CCFF00]">Health</span>
              </h2>
            </SR>
            <SR delay={0.05}>
              <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {SYSTEM_HEALTH.map((item, i) => (
                  <motion.div key={item.label}
                    ...
                    className={`bg-[rgba(20,20,20,0.4)] border rounded-2xl p-5 ${item.status === 'warning' ? 'border-[rgba(252,211,77,0.3)]' : 'border-[rgba(255,255,255,0.07)]'}`}>
                    <div className="flex items-center justify-between mb-3">
                      <span className="font-display font-bold text-[#888888] text-xs uppercase tracking-wide">{item.label}</span>
            ...
            <SR delay={0.12}>
              <div className="bg-[rgba(20,20,20,0.4)] border border-[rgba(255,255,255,0.07)] rounded-2xl p-6">
                ...
                <div className="grid sm:grid-cols-2 gap-4">
                  {[
                    ...
                    <div key={cfg.label} className="flex items-center justify-between px-4 py-3 bg-[rgba(12,12,12,0.5)] rounded-xl border border-[rgba(204,255,0,0.08)]">
                      <div>
                        <p className="font-display font-bold text-[#F2F2F2] text-xs uppercase tracking-wide">{cfg.label}</p>
                        <p className="font-sans text-[#888888] text-xs">{cfg.value}</p>
                      </div>
      ...
      <AnimatePresence>
        {toast && (
          <motion.div
            ...
            className="fixed bottom-6 right-6 z-[300] bg-[#141414] border border-[rgba(255,255,255,0.15)] rounded-xl px-5 py-3 flex items-center gap-3 shadow-card"
          >
            <CheckCircle size={15} className="text-[#6EE07A] flex-shrink-0" />
            <span className="font-display font-bold text-[#F2F2F2] text-sm uppercase tracking-wide">{toast}</span>
            <button onClick={() => setToast(null)} className="text-[#888888] hover:text-[#FCA5A5] transition-colors ml-1">
              <X size={13} />
            </button>
          </motion.div>
        )}
