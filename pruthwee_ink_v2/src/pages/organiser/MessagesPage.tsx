        ...
        <div className="flex items-start justify-between gap-4">
          <div>
            <h2 className="heading-gradient font-display font-black text-[#F2F2F2] uppercase leading-none"
              ...
              Volunteer <span className="text-[#CCFF00]">Messaging</span>
            </h2>
          ...
          <div className="flex items-center gap-2 bg-[rgba(255,255,255,0.04)] border border-[rgba(255,255,255,0.08)] rounded-xl px-4 py-2">
            ...
            <span className="font-display font-bold text-[#CCFF00] text-sm uppercase tracking-wide">{MOCK_REGISTRATIONS.length} volunteers</span>
          </div>
        </div>
        ...
        <div className="grid lg:grid-cols-[1fr_340px] gap-6">
          ...
          <div className="space-y-5">
            {sent ? (
              <motion.div initial={{ opacity: 0, scale: 0.97 }} animate={{ opacity: 1, scale: 1 }}
                ...
                <p className="font-sans text-[#888888] text-sm mb-2">
                  Your {channel.toUpperCase()} was dispatched to {audienceCount} volunteers.
                </p>
                <p className="font-mono text-[#CCFF00] text-xs">{new Date().toLocaleString()}</p>
              ...
              <form onSubmit={handleSend} className="space-y-5">
                ...
                <div className="bg-[rgba(20,20,20,0.4)] border border-[rgba(255,255,255,0.07)] rounded-2xl p-5">
                  ...
                  <p className="font-sans text-[#CCFF00] text-xs mt-2">
                    → <span className="font-bold text-[#CCFF00]">{audienceCount}</span> volunteers will receive this message.
                  </p>
                </div>
                    ...
                    {TEMPLATES.map(t => (
                      <button key={t.id} type="button" onClick={() => loadTemplate(t.id)}
                        className={`px-3 py-2 rounded-lg border text-left font-display font-bold text-xs uppercase tracking-wide transition-all ${
                          template === t.id
                            ? 'bg-[rgba(255,255,255,0.07)] border-[#CCFF00] text-[#CCFF00]'
                            : 'border-[rgba(255,255,255,0.07)] text-[#888888] hover:border-[rgba(255,255,255,0.12)]'
                        }`}>
                        {t.label}
                      </button>
                    ))}
                ...
                {/* Compose */}
                <div className="bg-[rgba(20,20,20,0.4)] border border-[rgba(255,255,255,0.07)] rounded-2xl p-5 space-y-4">
                  ...
                  <div>
                    ...
                    {channel === 'sms' && (
                      <p className={`font-mono text-xs mt-1 text-right ${body.length > 140 ? 'text-[#FCA5A5]' : 'text-[#CCFF00]'}`}>
                        {body.length}/160
                      </p>
                    )}
                  </div>
                  <p className="font-sans text-[#888888] text-xs">
                    Use <span className="font-mono text-[#CCFF00]">{'{name}'}</span> to personalise with each volunteer's name.
                  </p>
                </div>
          ...
          <div className="space-y-4">
            ...
            {SENT_MESSAGES.map(msg => (
              <div key={msg.id}
                className="bg-[rgba(20,20,20,0.4)] border border-[rgba(255,255,255,0.07)] rounded-xl p-4 hover:border-[rgba(255,255,255,0.12)] transition-colors">
                <div className="flex items-start gap-3 mb-3">
                  <div className={`w-7 h-7 rounded-lg flex items-center justify-center flex-shrink-0 mt-0.5 ${msg.channel === 'email' ? 'bg-[rgba(255,255,255,0.06)]' : 'bg-[rgba(196,181,253,0.1)]'}`}>
                    {msg.channel === 'email' ? <Mail size={14} className="text-[#CCFF00]" /> : <MessageSquare size={14} className="text-[#C4B5FD]" />}
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="heading-gradient font-display font-black text-[#F2F2F2] text-xs uppercase tracking-wide leading-tight line-clamp-2">
                      {msg.subject}
                    </p>
                    <p className="font-mono text-[#CCFF00] text-[10px] mt-1">{msg.sentAt}</p>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-3 gap-2 mb-3">
                  {[
                    { label: 'Sent',      value: msg.total,     color: '#CCFF00' },
                    { label: 'Delivered', value: msg.delivered, color: '#6EE07A' },
                    { label: 'Opened',    value: msg.opened,    color: '#FCD34D' },
                  ].map(s => (
                    <div key={s.label} className="text-center bg-[rgba(12,12,12,0.4)] rounded-lg py-2">
                      <div className="font-display font-black text-base leading-none" style={{ color: s.color }}>{s.value}</div>
                      <div className="font-mono text-[#888888] text-[9px] uppercase tracking-[1px] mt-0.5">{s.label}</div>
                    </div>
                  ))}
                </div>

                <button onClick={() => setPreview(msg)}
                  className="flex items-center gap-1.5 font-display font-bold text-[#CCFF00] text-[10px] uppercase tracking-wide hover:text-[#CCFF00] transition-colors">
        ...
        {preview && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <motion.div initial={{ opacity: 0, scale: 0.96 }} animate={{ opacity: 1, scale: 1 }}
              ...
              className="bg-[#141414] border border-[rgba(255,255,255,0.12)] rounded-2xl p-7 max-w-lg w-full"
              onClick={e => e.stopPropagation()}>
              <div className="flex items-center justify-between mb-4">
                <h3 className="heading-gradient font-display font-black text-[#F2F2F2] text-lg uppercase tracking-wide">Message Preview</h3>
                <button onClick={() => setPreview(null)} className="text-[#888888] hover:text-[#FCA5A5] transition-colors text-xl leading-none">×</button>
              </div>
              {preview.channel === 'email' && (
                <div className="mb-3 pb-3 border-b border-[rgba(204,255,0,0.1)]">
                  <span className="font-mono text-[#CCFF00] text-[10px] uppercase tracking-[2px]">Subject: </span>
                  <span className="font-sans text-[#F2F2F2] text-sm">{preview.subject}</span>
                </div>
              )}
              <pre className="font-sans text-[#888888] text-sm leading-relaxed whitespace-pre-wrap">{preview.body}</pre>
              <div className="mt-4 pt-4 border-t border-[rgba(204,255,0,0.1)] flex items-center gap-3">
                <span className="font-mono text-[#CCFF00] text-[10px] tracking-wider">{preview.sentAt}</span>
                <span className="font-mono text-[#888888] text-[10px]">→ {preview.audience}</span>
              </div>
            </motion.div>
          </motion.div>
        )}
