...
const CLOSE_STEPS = [
  { id: 1, label: 'Mark Attendance',  icon: <Users size={16} />,   color: '#CCFF00' },
  { id: 2, label: 'Log Hours',        icon: <Clock size={16} />,   color: '#CCFF00' },
      ...
      <div className="max-w-4xl mx-auto px-5 md:px-8 py-8 space-y-8">
        ...
        <div>
          <h2 className="heading-gradient font-display font-black text-[#F2F2F2] uppercase leading-none"
            ...
            Close <span className="text-[#CCFF00]">Event</span>
          </h2>
          ...
          {CLOSE_STEPS.map((s, i) => (
            <div key={s.id} className="flex items-center flex-1">
              <button onClick={() => step > s.id - 1 && setStep(s.id)}
                className={`flex items-center gap-2 px-4 py-3 rounded-xl border font-display font-bold text-xs uppercase tracking-wide transition-all flex-shrink-0 ${
                  step === s.id
                    ...
                    : 'border-[rgba(255,255,255,0.07)] text-[#888888]'
                }`}
              ...
              {i < CLOSE_STEPS.length - 1 && (
                <div className={`flex-1 h-0.5 mx-1 ${step > s.id ? 'bg-[#6EE07A]' : 'bg-[rgba(255,255,255,0.08)]'}`} />
              )}
            </div>
          ))}
        </div>
        ...
        {step === 1 && (
          <motion.div key="step1" initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="space-y-5">
            <div className="flex items-center justify-between">
              <div>
                <h3 className="heading-gradient font-display font-black text-[#F2F2F2] text-xl uppercase tracking-wide">Mark Attendance</h3>
              ...
              <div className="flex gap-2">
                ...
                <button onClick={() => setRows(p => p.map(r => ({ ...r, attended: false })))}
                  className="text-xs py-2 px-3 rounded-lg border border-[rgba(255,255,255,0.08)] text-[#888888] hover:text-[#FCA5A5] transition-colors">Clear All</button>
              </div>
            </div>
                ...
                <div key={row.regId}
                  className={`flex items-center gap-4 px-4 py-3 rounded-xl border transition-all ${
                    row.attended
                      ...
                      : 'bg-[rgba(12,12,12,0.5)] border-[rgba(204,255,0,0.08)]'
                  }`}>
                  <button onClick={() => toggleAttended(row.regId)}
                    className={`w-5 h-5 rounded border-2 flex items-center justify-center flex-shrink-0 transition-all ${
                      row.attended ? 'bg-[#6EE07A] border-[#6EE07A]' : 'border-[rgba(255,255,255,0.18)] hover:border-[#CCFF00]'
                    }`}>
                    {row.attended && <CheckCircle size={12} className="text-[#0C0C0C]" />}
                  </button>
                  <div className="flex-1 min-w-0">
                    <p className="heading-gradient font-display font-black text-[#F2F2F2] text-sm uppercase tracking-wide">{row.name}</p>
                    <p className="font-sans text-[#888888] text-xs">{row.sector} · {row.task} · {row.time}</p>
                  </div>
                  <span className={`font-mono text-xs tracking-wide ${row.attended ? 'text-[#6EE07A]' : 'text-[#888888]'}`}>
        ...
        {step === 2 && (
          <motion.div key="step2" initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="space-y-5">
            <div>
              <h3 className="heading-gradient font-display font-black text-[#F2F2F2] text-xl uppercase tracking-wide">Log Hours</h3>
              <p className="font-sans text-[#888888] text-sm mt-1">
            ...
            <div className="space-y-2 max-h-[480px] overflow-y-auto pr-1">
              {attended.map(row => (
                <div key={row.regId} className="flex items-center gap-4 px-4 py-3 rounded-xl bg-[rgba(20,20,20,0.4)] border border-[rgba(204,255,0,0.1)]">
                  <div className="flex-1 min-w-0">
                    <p className="heading-gradient font-display font-black text-[#F2F2F2] text-sm uppercase tracking-wide">{row.name}</p>
                    <p className="font-sans text-[#888888] text-xs">{row.sector} · {row.time}</p>
                  </div>
                  <div className="flex items-center gap-2 flex-shrink-0">
                    <span className="font-sans text-[#888888] text-xs">Planned: {row.planned}h</span>
                    <div className="flex items-center gap-1">
                      <button onClick={() => setActualHours(row.regId, Math.max(0, row.actual - 0.5))}
                        className="w-6 h-6 rounded bg-[rgba(255,255,255,0.06)] text-[#CCFF00] hover:bg-[rgba(204,255,0,0.2)] transition-colors font-bold text-sm">−</button>
                      <span className="font-display font-black text-[#CCFF00] text-base w-10 text-center">{row.actual}h</span>
        ...
        {step === 3 && (
          <motion.div key="step3" initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="space-y-5">
            <div>
              <h3 className="heading-gradient font-display font-black text-[#F2F2F2] text-xl uppercase tracking-wide">Issue Certificates</h3>
              <p className="font-sans text-[#888888] text-sm mt-1">
            ...
            <div className="bg-[rgba(20,20,20,0.4)] border border-[rgba(255,255,255,0.07)] rounded-2xl p-6">
              <p className="font-mono text-[#CCFF00] text-[10px] uppercase tracking-[2px] mb-4">Certificate Preview</p>
              <div className="bg-[#0C0C0C] border-2 border-[rgba(255,255,255,0.15)] rounded-xl p-8 text-center relative overflow-hidden">
                <div className="absolute top-3 left-3 w-6 h-6 border-t border-l border-[rgba(255,255,255,0.15)]" />
                <div className="absolute top-3 right-3 w-6 h-6 border-t border-r border-[rgba(255,255,255,0.15)]" />
                <div className="absolute bottom-3 left-3 w-6 h-6 border-b border-l border-[rgba(255,255,255,0.15)]" />
                <div className="absolute bottom-3 right-3 w-6 h-6 border-b border-r border-[rgba(255,255,255,0.15)]" />
                <p className="font-mono text-[#CCFF00] text-[10px] uppercase tracking-[3px] mb-1">Pruthwe volunteers trust</p>
                <p className="font-display font-black text-[#888888] text-xs uppercase tracking-[3px] mb-4">Certificate of Participation</p>
                <p className="font-sans text-[#888888] text-sm mb-1">This certifies that</p>
                <p className="heading-gradient font-display font-black text-[#F2F2F2] text-2xl uppercase mb-1">[Volunteer Name]</p>
                <p className="font-sans text-[#888888] text-sm mb-1">has successfully volunteered at</p>
                <p className="font-display font-black text-[#CCFF00] text-lg uppercase mb-1">{event.title}</p>
                <p className="font-sans text-[#888888] text-xs">Organised by Paryavaran Trust · {event.date}</p>
              </div>
            </div>

            {/* Cert options */}
            <div className="bg-[rgba(20,20,20,0.4)] border border-[rgba(255,255,255,0.07)] rounded-2xl p-5 space-y-3">
              <p className="font-display font-black text-[#CCFF00] text-xs uppercase tracking-[2px]">Certificate Options</p>
              {[
                ...
                <label key={i} className="flex items-center gap-3 cursor-pointer">
                  <div className="w-4 h-4 rounded border-2 border-[#CCFF00] bg-[rgba(255,255,255,0.06)] flex items-center justify-center flex-shrink-0">
                    {opt.checked && <CheckCircle size={10} className="text-[#CCFF00]" />}
                  </div>
                  <span className="font-sans text-[#888888] text-sm">{opt.label}</span>
                </label>
              ))}
            </div>
        ...
        {step === 4 && (
          <motion.div key="step4" initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="space-y-5">
            <div>
              <h3 className="heading-gradient font-display font-black text-[#F2F2F2] text-xl uppercase tracking-wide">Close Event</h3>
              <p className="font-sans text-[#888888] text-sm mt-1">Final review before permanently closing this event.</p>
            </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              {[
                ...
                { label: 'Total Hours',   value: `${totalHours}h`,  color: '#CCFF00' },
            ...
            {closed ? (
              ...
              <div className="flex justify-between pt-2">
                ...
                <button onClick={() => setClosed(true)}
                  className="bg-[#6EE07A] text-[#0C0C0C] font-display font-black text-sm uppercase tracking-wide px-8 py-3 rounded-xl flex items-center gap-2 hover:brightness-110 transition-all">
