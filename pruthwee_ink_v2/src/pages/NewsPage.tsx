        ...
        <div className="relative z-10 max-w-7xl mx-auto px-5 md:px-10">
          <motion.div initial={{ opacity: 0, y: 28 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
            ...
            <h1 className="heading-gradient font-display font-black text-[#F2F2F2] uppercase leading-none mb-4"
              ...
              News &{' '}
              <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#CCFF00] to-[#BBEE00]">Blog</span>
            </h1>
        ...
        <div className="max-w-7xl mx-auto px-5 md:px-10">
          <SR>
            <Link to={`/news/${featured.slug}`} className="block group">
              <div className="relative rounded-2xl overflow-hidden border border-[rgba(255,255,255,0.08)] hover:border-[rgba(255,255,255,0.18)] transition-all duration-300">
                ...
                <div className="absolute bottom-0 left-0 right-0 p-8 md:p-12">
                  <div className="flex items-center gap-3 mb-4">
                    ...
                    <span className="font-mono text-[#CCFF00] text-[10px] tracking-wider">{featured.date}</span>
                  </div>
                  <h2 className="heading-gradient font-display font-black text-[#F2F2F2] uppercase leading-tight group-hover:text-[#CCFF00] transition-colors"
                    style={{ fontSize: 'clamp(24px, 4vw, 48px)' }}>
                    {featured.title}
                  </h2>
                  <p className="font-sans text-[#888888] text-base mt-3 max-w-2xl leading-relaxed hidden md:block">
                    {featured.excerpt}
                  </p>
                  <div className="flex items-center gap-2 mt-4 font-display font-bold text-[#CCFF00] text-sm uppercase tracking-wide group-hover:gap-3 transition-all">
        ...
        <div className="max-w-7xl mx-auto px-5 md:px-10">
          ...
          <SR>
            <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-center justify-between mb-10">
              ...
              <div className="flex gap-2 flex-wrap">
                ...
                {(Object.keys(CATEGORY_META) as ArticleCategory[]).map(cat => (
                  <button
                    ...
                    className={`px-4 py-2 rounded-full border font-display font-bold text-xs uppercase tracking-wide transition-all ${
                      activeCategory === cat ? 'border-current' : 'border-[rgba(255,255,255,0.08)] text-[#888888] hover:border-[#CCFF00]'
                    }`}
              ...
              <div className="relative w-full sm:w-56">
                <Search size={15} className="absolute left-3 top-1/2 -translate-y-1/2 text-[#CCFF00]" />
                <input
                  type="text"
                  value={search}
          ...
          {rest.length === 0 ? (
            <div className="text-center py-16">
              ...
              <h3 className="heading-gradient font-display font-black text-[#F2F2F2] text-xl uppercase tracking-wide">No articles found</h3>
            </div>
          ) : (
            <motion.div
        ...
        <div className="h-full flex flex-col bg-[#141414] border border-[rgba(255,255,255,0.07)] rounded-2xl overflow-hidden hover:border-[rgba(255,255,255,0.18)] hover:-translate-y-1.5 transition-all duration-250 hover:shadow-card">
          ...
          <div className="flex-1 flex flex-col p-5">
            <div className="flex items-center gap-3 mb-3">
              <div className="flex items-center gap-1.5 font-sans text-[#888888] text-xs">
                <Calendar size={11} className="text-[#CCFF00]" /> {article.date}
              </div>
              <div className="flex items-center gap-1.5 font-sans text-[#888888] text-xs">
                <Clock size={11} className="text-[#CCFF00]" /> {article.readTime} read
              </div>
            </div>

            <h3 className="heading-gradient font-display font-black text-[#F2F2F2] text-base uppercase tracking-wide leading-tight mb-2 flex-1 group-hover:text-[#CCFF00] transition-colors">
              {article.title}
            </h3>
            <p className="font-sans text-[#888888] text-sm leading-relaxed mb-4 line-clamp-2">
              {article.excerpt}
            </p>
            ...
            <div className="flex flex-wrap gap-1.5 mb-4">
              {article.tags.slice(0, 3).map(tag => (
                <span key={tag} className="font-mono text-[#CCFF00] text-[9px] tracking-[1.5px] uppercase border border-[rgba(255,255,255,0.08)] px-2 py-0.5 rounded-full">
                  {tag}
                </span>
              ))}
            </div>
            ...
            <div className="flex items-center justify-between pt-3 border-t border-[rgba(204,255,0,0.1)]">
              <span className="font-sans text-[#888888] text-xs">{article.author}</span>
              <span className="flex items-center gap-1 font-display font-bold text-[#CCFF00] text-xs uppercase tracking-wide group-hover:gap-2 transition-all group-hover:text-[#CCFF00]">
