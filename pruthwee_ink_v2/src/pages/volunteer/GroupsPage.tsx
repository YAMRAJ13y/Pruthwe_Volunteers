...
const MY_GROUPS: Group[] = [
  {
    ...
    color:     '#6EE07A',
    members: [
      { name: 'Arjun Patel',    initials: 'AP', color: '#CCFF00', role: 'leader' },
  ...
  },
  {
    ...
    color:     '#CCFF00',
    members: [
      { name: 'Sneha Rathi',    initials: 'SR', color: '#CCFF00', role: 'leader' },
      { name: 'Arjun Patel',    initials: 'AP', color: '#CCFF00', role: 'member' },
      ...
      { name: 'Janhvi Modi',    initials: 'JM', color: '#CCFF00', role: 'member' },
    ],
  },
];
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
      ...
      <motion.div initial={{ opacity: 0, scale: 0.95, y: 16 }} animate={{ opacity: 1, scale: 1, y: 0 }}
        ...
        className="bg-[#141414] border border-[rgba(255,255,255,0.12)] rounded-2xl p-8 w-full max-w-sm"
        onClick={e => e.stopPropagation()}>

        <div className="flex items-center justify-between mb-6">
          <h3 className="heading-gradient font-display font-black text-[#F2F2F2] text-xl uppercase tracking-wide">Join a Group</h3>
          <button onClick={onClose} className="text-[#888888] hover:text-[#FCA5A5] transition-colors"><X size={18} /></button>
        </div>
        ...
        {done ? (
          <div className="text-center py-4">
            ...
            <p className="font-sans text-[#888888] text-sm mb-5">You've been added to the group. Your group leader will be notified.</p>
            <button onClick={onClose} className="btn-primary w-full justify-center">Done</button>
          </div>
        ) : (
          <form onSubmit={handleJoin} className="space-y-4">
            <p className="font-sans text-[#888888] text-sm leading-relaxed">
              Enter the group token shared by your group leader. Tokens look like <span className="font-mono text-[#CCFF00]">XXX-YYYY-ZZZZZ</span>.
            </p>
            <div>
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
      ...
      <motion.div initial={{ opacity: 0, scale: 0.95, y: 16 }} animate={{ opacity: 1, scale: 1, y: 0 }}
        ...
        <div className="flex items-center justify-between mb-6">
          <h3 className="heading-gradient font-display font-black text-[#F2F2F2] text-xl uppercase tracking-wide">Create Group</h3>
          <button onClick={onClose} className="text-[#888888] hover:text-[#FCA5A5] transition-colors"><X size={18} /></button>
        </div>
        ...
        {done ? (
          <div className="space-y-4">
            <div className="text-center">
              ...
              <p className="font-sans text-[#888888] text-sm mb-4">Share the token below with your group members.</p>
            </div>
            <div className="bg-[rgba(12,12,12,0.5)] border border-[rgba(255,255,255,0.08)] rounded-xl p-4 flex items-center justify-between gap-3">
              <span className="font-mono text-[#CCFF00] text-base tracking-widest">{generatedToken}</span>
              <button onClick={copyToken} className="text-[#CCFF00] hover:text-[#CCFF00] transition-colors flex-shrink-0">
                <Copy size={16} />
              </button>
            </div>
            <button onClick={onClose} className="btn-primary w-full justify-center">Done</button>
      ...
      <div className="max-w-4xl mx-auto px-4 md:px-8 py-8 space-y-6">
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            <div>
              ...
              <h1 className="heading-gradient font-display font-black text-[#F2F2F2] uppercase leading-none"
                ...
                My <span className="text-[#CCFF00]">Groups</span>
              </h1>
            </div>
        ...
        <SR delay={0.07}>
          <div className="grid grid-cols-3 gap-4">
            {[
              { label: 'Groups',          value: MY_GROUPS.length,                                       color: '#CCFF00' },
        ...
        {/* How groups work */}
        <SR delay={0.09}>
          <div className="bg-[rgba(204,255,0,0.05)] border border-[rgba(255,255,255,0.08)] rounded-2xl p-5 grid sm:grid-cols-3 gap-4">
            {[
              ...
              <div key={item.title} className="flex items-start gap-3">
                ...
                <div>
                  <div className="font-display font-black text-[#CCFF00] text-xs uppercase tracking-wide mb-1">{item.title}</div>
                  <div className="font-sans text-[#888888] text-xs leading-relaxed">{item.desc}</div>
                </div>
              </div>
            ))}
          </div>
        ...
        <SR delay={0.11}>
          {MY_GROUPS.length === 0 ? (
            <div className="text-center py-16 bg-[rgba(20,20,20,0.3)] rounded-2xl border border-[rgba(204,255,0,0.1)]">
              <Users size={36} className="text-[#CCFF00] mx-auto mb-3 opacity-50" />
              <h3 className="heading-gradient font-display font-black text-[#F2F2F2] text-lg uppercase tracking-wide mb-1">No Groups Yet</h3>
              <p className="font-sans text-[#888888] text-sm">Create a group or join one with a token.</p>
            </div>
          ) : (
            <div className="space-y-4">
                  ...
                  >
                        ...
                        <div className="flex flex-wrap items-center gap-2 mb-0.5">
                          ...
                          <span className={`text-[9px] font-mono uppercase tracking-[1.5px] px-2 py-0.5 rounded-full border ${
                            group.status === 'active'
                              ...
                              : 'text-[#888888] border-[rgba(255,255,255,0.1)]'
                          }`}>
                            {group.status}
                          </span>
                        </div>
                        <p className="font-sans text-[#888888] text-xs line-clamp-1">
                          {group.event} · {group.eventDate}
                        </p>
                        <div className="flex items-center gap-2 mt-1.5">
                          <div className="flex -space-x-2">
                            {group.members.slice(0, 5).map((m, mi) => (
                              <div key={mi} className="w-6 h-6 rounded-full flex items-center justify-center text-[#0C0C0C] text-[9px] font-black border border-[#0C0C0C]"
                                style={{ background: m.color, zIndex: 5 - mi }}>
                                {m.initials}
                              </div>
                            ))}
                          </div>
                          <span className="font-sans text-[#888888] text-[11px]">
                            {group.members.length} member{group.members.length !== 1 ? 's' : ''}
                          </span>
                        </div>
                      </div>

                      {isOpen ? <ChevronUp size={16} className="text-[#CCFF00] flex-shrink-0" /> : <ChevronDown size={16} className="text-[#CCFF00] flex-shrink-0" />}
                    </button>
                    ...
                    <AnimatePresence>
                      {isOpen && (
                        ...
                        >
                          <div className="px-5 pb-5 border-t border-[rgba(204,255,0,0.1)] space-y-5 pt-4">

                            {/* Token (leader only) */}
                            {group.role === 'leader' && (
                              <div>
                                <label className="label mb-2 block">Group Token</label>
                                <div className="flex items-center gap-3 bg-[rgba(12,12,12,0.5)] border border-[rgba(255,255,255,0.08)] rounded-xl px-4 py-3">
                                  <Hash size={14} className="text-[#CCFF00]" />
                                  <span className="font-mono text-[#CCFF00] text-sm tracking-widest flex-1">{group.token}</span>
                                  <button onClick={() => copyToken(group.token)}
                                    className="text-[#CCFF00] hover:text-[#CCFF00] transition-colors">
                            ...
                            <div>
                              ...
                              <div className="grid sm:grid-cols-2 gap-2">
                                {group.members.map((member) => (
                                  <div key={member.name}
                                    className="flex items-center gap-3 px-3 py-2.5 rounded-xl bg-[rgba(12,12,12,0.4)] border border-[rgba(204,255,0,0.08)]">
                                    <div className="w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 text-[#0C0C0C] text-xs font-black"
                                    ...
                                    <div className="flex-1 min-w-0">
                                      ...
                                      <p className="font-mono text-[#CCFF00] text-[9px] uppercase tracking-[1px]">
                                        {member.role}
                                      </p>
                                    </div>
                                  </div>
                            ...
                            <div className="flex flex-wrap gap-2">
                              <Link to={`/events/1`}
                                className="flex items-center gap-2 px-4 py-2 rounded-lg bg-[rgba(255,255,255,0.06)] border border-[rgba(255,255,255,0.12)] text-[#CCFF00] font-display font-bold text-xs uppercase tracking-wide hover:bg-[rgba(204,255,0,0.18)] transition-all">
