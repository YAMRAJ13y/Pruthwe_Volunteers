...
const RECENT_ACTIVITY = [
  ...
  { icon: '📄', text: 'Certificate issued for Coastal Clean-Up 2025',   date: 'Jan 28, 2026', color: '#CCFF00' },
  { icon: '✅', text: 'Confirmed for Sabarmati Clean-Up 2026',           date: 'Jan 22, 2026', color: '#6EE07A' },
  { icon: '📋', text: 'Registered for Tree Plantation Drive',            date: 'Jan 18, 2026', color: '#CCFF00' },
...
const STATUS_COLORS: Record<string, { bg: string; text: string; border: string; label: string }> = {
  ...
  allocated: { bg: 'rgba(204,255,0,0.1)', text: '#CCFF00', border: 'rgba(204,255,0,0.3)', label: 'Allocated' },
  completed: { bg: 'rgba(204,255,0,0.1)',  text: '#CCFF00', border: 'rgba(204,255,0,0.3)',  label: 'Completed' },
};
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-8 py-8 space-y-8">
        ...
        <SR>
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            <div>
              <p className="font-mono text-[#CCFF00] text-xs tracking-[2px] uppercase mb-1">Welcome back</p>
              ...
              <div className="flex items-center gap-2 mt-2">
                ...
                <span className="font-sans text-[#888888] text-xs">{v.city}</span>
              </div>
            </div>
        ...
        <SR delay={0.07}>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {[
              { icon: <Clock size={18} />,    label: 'Total Hours',   value: `${v.totalHours}h`,     color: '#CCFF00', sub: `+14.5h this month`    },
              { icon: <Calendar size={18} />, label: 'Events',        value: v.eventsAttended,        color: '#CCFF00', sub: `3 upcoming`            },
              ...
              <motion.div key={stat.label}
                ...
                className="bg-[rgba(20,20,20,0.5)] border border-[rgba(255,255,255,0.07)] rounded-2xl p-5 hover:border-[rgba(255,255,255,0.15)] transition-colors">
                ...
                <div className="font-display font-bold text-[#888888] text-xs uppercase tracking-[2px] mb-1">{stat.label}</div>
                <div className="font-sans text-[#CCFF00] text-[11px]">{stat.sub}</div>
              </motion.div>
            ))}
          </div>
        </SR>
        ...
        <div className="grid md:grid-cols-[1fr_340px] gap-5">
          ...
          <SR delay={0.1}>
            <div className="bg-[rgba(20,20,20,0.4)] border border-[rgba(255,255,255,0.07)] rounded-2xl p-6">
              <div className="flex items-center justify-between mb-6">
                <div>
                  <h3 className="heading-gradient font-display font-black text-[#F2F2F2] text-base uppercase tracking-wide">Hours Chart</h3>
                  <p className="font-sans text-[#888888] text-xs">Aug 2025 – Feb 2026</p>
                </div>
              ...
              <div className="flex items-end gap-3 h-36">
                {HOURS_BY_MONTH.map((m, i) => (
                  <div key={m.month} className="flex-1 flex flex-col items-center gap-1.5">
                    <span className="font-sans text-[#CCFF00] text-[10px]">{m.hours > 0 ? `${m.hours}h` : ''}</span>
                    <motion.div
                      initial={{ height: 0 }}
                      ...
                      style={{
                        background: m.month === 'Feb' || m.month === 'Jan'
                          ? 'linear-gradient(to top, #CCFF00, #CCFF00)'
                          : 'rgba(204,255,0,0.25)',
                      }}
                    />
                    <span className="font-display font-bold text-[#888888] text-[10px] uppercase">{m.month}</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          {/* Tier progress */}
          <SR delay={0.12}>
            <div className="bg-[rgba(20,20,20,0.4)] border border-[rgba(255,255,255,0.07)] rounded-2xl p-6 flex flex-col">
              <h3 className="heading-gradient font-display font-black text-[#F2F2F2] text-base uppercase tracking-wide mb-5">Tier Progress</h3>
              ...
              <div className="flex items-center gap-4 mb-6 p-4 rounded-xl border" style={{ borderColor: tier.borderColor, background: tier.bgColor }}>
                ...
                <div>
                  ...
                  <div className="font-sans text-[#888888] text-xs">{v.totalHours}h total</div>
                </div>
              </div>
              ...
              {nextTier && (
                <div className="mb-5">
                  <div className="flex justify-between mb-2">
                    <span className="font-display font-bold text-[#888888] text-xs uppercase tracking-wide">{tier.label}</span>
                  ...
                  <p className="font-sans text-[#CCFF00] text-xs mt-2">
                    <span className="font-black text-[#CCFF00]">{hoursToNext.toFixed(1)}h</span> more to {nextTier.label}
                  </p>
                </div>
              )}
                ...
                {STATUS_TIERS.slice(1).map((t) => (
                  <div key={t.id} className="flex items-center gap-2">
                    ...
                    <div className="flex-1 h-1 rounded-full" style={{
                      background: v.totalHours >= t.hoursMin ? t.color : 'rgba(204,255,0,0.1)'
                    }} />
        ...
        <div className="grid lg:grid-cols-[1fr_320px] gap-5">
          ...
          <SR delay={0.14}>
            <div className="bg-[rgba(20,20,20,0.4)] border border-[rgba(255,255,255,0.07)] rounded-2xl overflow-hidden">
              <div className="flex items-center justify-between px-6 py-5 border-b border-[rgba(204,255,0,0.1)]">
                <h3 className="heading-gradient font-display font-black text-[#F2F2F2] text-base uppercase tracking-wide">
                  Upcoming Assignments
                </h3>
                <Link to="/dashboard/assignments"
                  className="flex items-center gap-1.5 font-display font-bold text-[#CCFF00] text-xs uppercase tracking-wide hover:text-[#CCFF00] transition-colors">
                  View All <ChevronRight size={13} />
                </Link>
              </div>

              <div className="divide-y divide-[rgba(204,255,0,0.07)]">
                {UPCOMING_ASSIGNMENTS.map((a) => {
                  const sc = STATUS_COLORS[a.status];
                  return (
                    <motion.div key={a.id}
                      whileHover={{ backgroundColor: 'rgba(204,255,0,0.04)', transition: { duration: 0.1 } }}
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-start justify-between gap-2">
                          <p className="heading-gradient font-display font-black text-[#F2F2F2] text-sm uppercase tracking-wide leading-tight line-clamp-1">
                            {a.event}
                          </p>
                        ...
                        <p className="font-sans text-[#CCFF00] text-xs mt-0.5">{a.sector}</p>
                        <div className="flex flex-wrap gap-x-3 gap-y-0.5 mt-1.5">
                          <span className="flex items-center gap-1 font-sans text-[#888888] text-[11px]">
                            <Calendar size={10} className="text-[#CCFF00]" />{a.date}
                          </span>
                          <span className="flex items-center gap-1 font-sans text-[#888888] text-[11px]">
                            <Clock size={10} className="text-[#CCFF00]" />{a.time}
                          </span>
                          <span className="flex items-center gap-1 font-sans text-[#888888] text-[11px]">
                            <MapPin size={10} className="text-[#CCFF00]" />{a.venue}
                          </span>
                        </div>
                      </div>
                    </motion.div>
              ...
              <div className="px-6 py-4 border-t border-[rgba(204,255,0,0.07)]">
                <Link to="/events" className="flex items-center gap-2 font-display font-bold text-[#CCFF00] text-xs uppercase tracking-wide hover:text-[#CCFF00] transition-colors">
                  ...
                  {RECENT_ACTIVITY.map((item, i) => (
                    <div key={i} className="flex items-start gap-3 px-5 py-3">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="font-mono text-[#CCFF00] text-[10px] mt-0.5">{item.date}</p>
                      </div>
                    </div>
                  ))}
                </div>
            ...
            <SR delay={0.18}>
              <div className="bg-[rgba(20,20,20,0.4)] border border-[rgba(255,255,255,0.07)] rounded-2xl p-4 space-y-2">
                {[
                  ...
                  { icon: <Download size={15} />, label: 'Download Profile', to: '#',                       color: '#CCFF00' },
                ].map((item) => (
                  <Link key={item.label} to={item.to}
                    className="flex items-center gap-3 px-3 py-2.5 rounded-xl hover:bg-[rgba(255,255,255,0.04)] transition-colors group">
                    <div className="w-7 h-7 rounded-lg flex items-center justify-center flex-shrink-0"
                      style={{ background: `${item.color}18`, color: item.color }}>
                      {item.icon}
                    </div>
                    <span className="font-display font-bold text-[#888888] text-xs uppercase tracking-wide group-hover:text-[#F2F2F2] transition-colors flex-1">
                      {item.label}
                    </span>
                    <ChevronRight size={13} className="text-[#CCFF00] opacity-0 group-hover:opacity-100 transition-opacity" />
                  </Link>
                ))}
              </div>
            </SR>
            ...
            <SR delay={0.2}>
              <div className="bg-[rgba(110,224,122,0.05)] border border-[rgba(110,224,122,0.2)] rounded-2xl p-5">
                ...
                <div className="flex items-end gap-3 mb-3">
                  <span className="heading-gradient font-display font-black text-[#F2F2F2] leading-none" style={{ fontSize: 48 }}>
                    {v.mqScore}
                  </span>
                  <span className="font-display font-bold text-[#888888] text-sm mb-2">/ 100</span>
                </div>
                ...
                <p className="font-sans text-[#888888] text-[11px] leading-snug">
        ...
        <SR delay={0.2}>
          <div className="grid sm:grid-cols-2 gap-5">
            <div className="bg-[rgba(20,20,20,0.4)] border border-[rgba(255,255,255,0.07)] rounded-2xl p-5">
              <div className="flex items-center justify-between mb-4">
                <h3 className="heading-gradient font-display font-black text-[#F2F2F2] text-sm uppercase tracking-wide">My Skills</h3>
                <Link to="/profile" className="font-display font-bold text-[#CCFF00] text-[10px] uppercase tracking-wide hover:text-[#CCFF00] transition-colors">Edit</Link>
              </div>
            ...
            <div className="bg-[rgba(20,20,20,0.4)] border border-[rgba(255,255,255,0.07)] rounded-2xl p-5">
              <div className="flex items-center justify-between mb-4">
                <h3 className="heading-gradient font-display font-black text-[#F2F2F2] text-sm uppercase tracking-wide">My Causes</h3>
                <Link to="/profile" className="font-display font-bold text-[#CCFF00] text-[10px] uppercase tracking-wide hover:text-[#CCFF00] transition-colors">Edit</Link>
              </div>
              <div className="flex flex-wrap gap-2">
                {v.causes.map(c => (
                  <span key={c} className="badge" style={{ color: '#CCFF00', background: 'rgba(204,255,0,0.1)', borderColor: 'rgba(204,255,0,0.25)' }}>
                    {c}
                  </span>
                ))}
                <Link to="/profile" className="badge border-dashed border-[rgba(255,255,255,0.15)] text-[#CCFF00] hover:border-[#CCFF00] transition-colors">
                  + Add Cause
                </Link>
              </div>
            </div>
