...
const STATUS_META: Record<string, { label: string; bg: string; text: string; border: string; icon: React.ReactNode }> = {
  ...
  allocated: { label: 'Allocated', bg: 'rgba(204,255,0,0.1)', text: '#CCFF00', border: 'rgba(204,255,0,0.3)', icon: <AlertCircle size={11} /> },
  completed: { label: 'Completed', bg: 'rgba(204,255,0,0.1)',   text: '#CCFF00', border: 'rgba(204,255,0,0.3)',   icon: <CheckCircle size={11} /> },
      ...
      <div className="max-w-5xl mx-auto px-4 md:px-8 py-8 space-y-6">
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            <div>
              ...
              <h1 className="heading-gradient font-display font-black text-[#F2F2F2] uppercase leading-none"
                ...
                My <span className="text-[#CCFF00]">Assignments</span>
              </h1>
            </div>
        ...
        <SR delay={0.07}>
          <div className="grid grid-cols-3 gap-4">
            {[
              { label: 'Total Assignments', value: ALL_ASSIGNMENTS.length, color: '#CCFF00' },
        ...
        <SR delay={0.1}>
          <div className="flex flex-col sm:flex-row gap-3 items-start sm:items-center">
              ...
              {TABS.map(t => (
                <button key={t.id} onClick={() => setTab(t.id)}
                  className={`px-4 py-2 rounded-lg font-display font-bold text-xs uppercase tracking-wide transition-all duration-150 ${
                    tab === t.id ? 'bg-[rgba(255,255,255,0.08)] text-[#CCFF00] shadow-inner-teal' : 'text-[#888888] hover:text-[#CCFF00]'
                  }`}>
                  {t.label}
            ...
            <span className="font-mono text-[#CCFF00] text-xs ml-auto">
        ...
        <SR delay={0.13}>
          {filtered.length === 0 ? (
            <div className="text-center py-16 bg-[rgba(20,20,20,0.3)] rounded-2xl border border-[rgba(204,255,0,0.1)]">
              <FileText size={36} className="text-[#CCFF00] mx-auto mb-3 opacity-50" />
              <h3 className="heading-gradient font-display font-black text-[#F2F2F2] text-lg uppercase tracking-wide mb-1">No Assignments Found</h3>
              ...
              {filtered.map((a, i) => {
                ...
                return (
                  <motion.div
                    ...
                    className="bg-[rgba(20,20,20,0.4)] border border-[rgba(255,255,255,0.07)] rounded-2xl overflow-hidden hover:border-[rgba(255,255,255,0.12)] transition-colors"
                  >
                    {/* Main row */}
                    <button
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="font-sans text-[#CCFF00] text-xs mt-0.5">{a.sector} · {a.task}</p>
                      ...
                      <div className="flex-shrink-0">
                        {isOpen ? <ChevronUp size={16} className="text-[#CCFF00]" /> : <ChevronDown size={16} className="text-[#CCFF00]" />}
                      </div>
                    </button>
                    ...
                    <AnimatePresence>
                      {isOpen && (
                        ...
                        >
                          <div className="px-4 pb-5 pt-0 border-t border-[rgba(204,255,0,0.1)] space-y-4">
                            {/* Details grid */}
                            <div className="grid sm:grid-cols-2 gap-3 pt-4">
                              <div className="bg-[rgba(12,12,12,0.5)] rounded-xl p-4 space-y-2">
                                <p className="font-mono text-[#CCFF00] text-[10px] uppercase tracking-[2px] mb-2">Event Details</p>
                                <p className="font-display font-bold text-[#888888] text-xs">Organiser: <span className="text-[#F2F2F2]">{a.org}</span></p>
                                <div className="flex items-start gap-2">
                                  <MapPin size={12} className="text-[#CCFF00] mt-0.5 flex-shrink-0" />
                                  <p className="font-sans text-[#888888] text-xs leading-snug">{a.venue}</p>
                                </div>
                              </div>
                              <div className="bg-[rgba(12,12,12,0.5)] rounded-xl p-4 space-y-2">
                                <p className="font-mono text-[#CCFF00] text-[10px] uppercase tracking-[2px] mb-2">Assignment</p>
                                <p className="font-display font-bold text-[#888888] text-xs">Sector: <span className="text-[#F2F2F2]">{a.sector}</span></p>
                                <p className="font-display font-bold text-[#888888] text-xs">Task: <span className="text-[#F2F2F2]">{a.task}</span></p>
                                {a.hours > 0 && (
                                  <p className="font-display font-bold text-[#888888] text-xs">Hours: <span className="text-[#6EE07A]">{a.hours}h</span></p>
                                )}
                              </div>
                            </div>
                            ...
                            <div className="flex flex-wrap gap-2">
                              {a.hasMission && (
                                <button className="flex items-center gap-2 px-4 py-2 rounded-lg bg-[rgba(255,255,255,0.06)] border border-[rgba(255,255,255,0.1)] text-[#CCFF00] font-display font-bold text-xs uppercase tracking-wide hover:bg-[rgba(204,255,0,0.18)] transition-all">
                              ...
                              <Link to={`/events/1`}
                                className="flex items-center gap-2 px-4 py-2 rounded-lg border border-[rgba(255,255,255,0.08)] text-[#888888] font-display font-bold text-xs uppercase tracking-wide hover:border-[rgba(204,255,0,0.35)] hover:text-[#CCFF00] transition-all">
