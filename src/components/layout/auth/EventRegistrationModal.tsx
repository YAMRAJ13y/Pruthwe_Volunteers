  ...
  return (
    <div className="flex flex-col items-center gap-1 flex-1">
      <div className={`w-7 h-7 rounded-full flex items-center justify-center border-2 transition-all duration-300 ${
        ...
        : active ? 'bg-[rgba(84,172,191,0.15)] border-[#54ACBF]'
        :          'bg-transparent border-[rgba(84,172,191,0.2)]'
      }`}>
        {done
          ? <CheckCircle size={14} className="text-[#011C40]" />
          : <span className={`font-mono text-xs font-bold ${active ? 'text-[#A7EBF2]' : 'text-[rgba(84,172,191,0.35)]'}`}>{index + 1}</span>
        }
      </div>
      <span className={`font-mono text-[9px] uppercase tracking-[1.5px] hidden sm:block ${
        active ? 'text-[#A7EBF2]' : done ? 'text-[#6EE07A]' : 'text-[rgba(84,172,191,0.35)]'
      }`}>{label}</span>
    </div>
  );
}
  ...
  return (
    <button type="button" onClick={onClick}
      className={`w-full text-left p-4 rounded-xl border-2 transition-all duration-150 ${
        selected
          ? 'border-[#54ACBF] bg-[rgba(84,172,191,0.1)]'
          : 'border-[rgba(84,172,191,0.12)] hover:border-[rgba(84,172,191,0.3)]'
      }`}>
      <div className="flex items-center justify-between">
        <span className={`font-display font-black text-sm uppercase tracking-wide ${selected ? 'text-[#A7EBF2]' : 'text-[#8BBFCC]'}`}>
          {label}
        </span>
        {selected && <CheckCircle size={15} className="text-[#54ACBF] flex-shrink-0" />}
      </div>
      <p className={`font-sans text-xs mt-0.5 ${selected ? 'text-[#8BBFCC]' : 'text-[rgba(139,191,204,0.6)]'}`}>{desc}</p>
    </button>
  );
}
...
function SectorChip({ label, selected, onClick }: { label: string; selected: boolean; onClick: () => void }) {
  return (
    <button type="button" onClick={onClick}
      ...
      {selected && <span className="mr-1 text-[#54ACBF]">✓</span>}{label}
    </button>
  );
}
      ...
      {open && (
        <>
          ...
          >
            ...
            >
              <div className="bg-[#023859] border border-[rgba(84,172,191,0.2)] rounded-2xl shadow-[0_32px_80px_rgba(1,28,64,0.9)] flex flex-col overflow-hidden">

                {/* Progress bar */}
                <div className="h-1 bg-[rgba(84,172,191,0.08)] flex-shrink-0">
                  <motion.div
                    className="h-full bg-gradient-to-r from-[#54ACBF] via-[#A7EBF2] to-[#6EE07A]"
                ...
                {/* Header */}
                <div className="flex-shrink-0 px-6 pt-5 pb-4 border-b border-[rgba(84,172,191,0.08)]">
                  <div className="flex items-start justify-between gap-3 mb-3">
                    <div className="min-w-0">
                      <p className="font-mono text-[#54ACBF] text-[10px] tracking-[3px] uppercase mb-0.5">
                        Event Registration
                      </p>
                      <h2 className="font-display font-black text-[#F0FAFB] uppercase text-xl leading-tight tracking-tight line-clamp-2">
                        {event.title}
                      </h2>
                      <div className="flex flex-wrap items-center gap-x-3 gap-y-1 mt-1.5">
                        <span className="flex items-center gap-1 font-sans text-[#8BBFCC] text-xs">
                          <Calendar size={11} className="text-[#54ACBF]" />{event.date}
                        </span>
                        <span className="flex items-center gap-1 font-sans text-[#8BBFCC] text-xs">
                          <MapPin size={11} className="text-[#54ACBF]" />{event.city}
                        </span>
                    ...
                    {step < 4 && (
                      <button onClick={close}
                        className="w-8 h-8 rounded-lg flex-shrink-0 flex items-center justify-center text-[#8BBFCC] hover:text-[#FCA5A5] hover:bg-[rgba(252,165,165,0.08)] transition-all"
                        aria-label="Close">
                        <X size={18} />
                      </button>
                    )}
                        ...
                        <div key={label} className="flex items-center flex-1">
                          ...
                          {i < STEP_LABELS.length - 1 && (
                            <div className={`h-px flex-1 mb-5 mx-1 transition-all duration-300 ${
                              i < step ? 'bg-[#6EE07A]' : 'bg-[rgba(84,172,191,0.15)]'
                            }`} />
                          )}
                        </div>
                      ))}
                ...
                <div className="flex-1 overflow-y-auto px-6 py-5 space-y-4">
                  ...
                  {step === 0 && (
                    <motion.div key="s0" initial={{ opacity: 0, x: 16 }} animate={{ opacity: 1, x: 0 }} className="space-y-4">
                      ...
                      <div className="bg-[rgba(1,28,64,0.5)] border border-[rgba(84,172,191,0.1)] rounded-xl p-4">
                        <div className="grid grid-cols-2 gap-3">
                          {[
                            ...
                            <div key={r.label}>
                              <div className="flex items-center gap-1.5 text-[#54ACBF] mb-0.5">
                                {r.icon}
                                <span className="font-mono text-[10px] uppercase tracking-[2px]">{r.label}</span>
                              </div>
                              <p className="font-display font-bold text-[#F0FAFB] text-sm uppercase tracking-wide">{r.value}</p>
                            </div>
                          ))}
                        </div>
                      </div>
                      ...
                      <div className="bg-[rgba(110,224,122,0.05)] border border-[rgba(110,224,122,0.12)] rounded-xl p-4">
                        ...
                        <ol className="space-y-1.5">
                          {['Submit registration', 'Organiser reviews & approves', 'Receive sector assignment', 'Attend & log hours', 'Get your certificate'].map((s, i) => (
                            <li key={i} className="flex items-center gap-2.5 font-sans text-[#8BBFCC] text-xs">
                      ...
                      {!user ? (
                        ...
                        <div className="flex items-center gap-3 bg-[rgba(84,172,191,0.07)] border border-[rgba(84,172,191,0.15)] rounded-xl px-4 py-3">
                          <div className="w-8 h-8 rounded-lg bg-[rgba(84,172,191,0.15)] flex items-center justify-center font-display font-black text-[#A7EBF2] text-xs flex-shrink-0">
                            {(profile?.fullName ?? user?.email ?? 'V').split(' ').map((p: string) => p[0]).join('').slice(0, 2)}
                          </div>
                          <div className="flex-1 min-w-0">
                            <p className="font-display font-bold text-[#F0FAFB] text-sm uppercase tracking-wide truncate">{profile.fullName}</p>
                            <p className="font-sans text-[#8BBFCC] text-xs">{profile.city} · {user.email}</p>
                          </div>
                  ...
                  {step === 1 && (
                    <motion.div key="s1" initial={{ opacity: 0, x: 16 }} animate={{ opacity: 1, x: 0 }} className="space-y-3">
                      <p className="font-sans text-[#8BBFCC] text-sm">When can you volunteer at this event?</p>
                    ...
                    <motion.div key="s2" initial={{ opacity: 0, x: 16 }} animate={{ opacity: 1, x: 0 }} className="space-y-5">
                      ...
                      <div>
                        <label className="label mb-1 block" htmlFor="group-token">
                          Group Token
                          <span className="font-sans text-[#8BBFCC] text-xs font-normal ml-2 normal-case tracking-normal">— optional</span>
                        </label>
                        <div className="relative">
                          <Hash size={14} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-[#54ACBF]" />
                          <input
                  ...
                  {step === 3 && (
                    <motion.div key="s3" initial={{ opacity: 0, x: 16 }} animate={{ opacity: 1, x: 0 }} className="space-y-4">
                      <p className="font-sans text-[#8BBFCC] text-sm">Review your registration before submitting.</p>
                      <div className="bg-[rgba(1,28,64,0.5)] border border-[rgba(84,172,191,0.1)] rounded-xl divide-y divide-[rgba(84,172,191,0.07)]">
                        {([
                          ...
                          <div key={row.label} className="flex items-start gap-4 px-4 py-3">
                            <span className="font-mono text-[#54ACBF] text-[10px] uppercase tracking-[2px] flex-shrink-0 w-24 pt-0.5">{row.label}</span>
                            <span className="font-sans text-[#F0FAFB] text-sm flex-1 break-words">{row.value}</span>
                          </div>
                        ))}
                      </div>
                      <div className="flex items-start gap-2.5 bg-[rgba(110,224,122,0.05)] border border-[rgba(110,224,122,0.12)] rounded-xl p-3.5">
                        ...
                        <p className="font-sans text-[#8BBFCC] text-xs leading-relaxed">
                  ...
                  {step === 4 && (
                    <motion.div key="s4" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }}
                      ...
                      <motion.div
                        ...
                        style={{ background: 'radial-gradient(circle, #6EE07A, #54ACBF)', boxShadow: '0 0 40px rgba(110,224,122,0.35)' }}
                      >
                        <CheckCircle size={38} className="text-[#011C40]" />
                      </motion.div>

                      <div>
                        <h3 className="font-display font-black text-[#F0FAFB] text-2xl uppercase tracking-tight mb-1">
                          You're Registered!
                        </h3>
                        <p className="font-sans text-[#8BBFCC] text-sm leading-relaxed max-w-xs mx-auto">
                          Registration for <span className="text-[#A7EBF2] font-medium">{event.title}</span> submitted.
                          The organiser will confirm your spot shortly.
                        </p>
                      </div>

                      <div className="bg-[rgba(84,172,191,0.07)] border border-[rgba(84,172,191,0.2)] rounded-xl px-5 py-4 inline-block mx-auto">
                        <p className="font-mono text-[#54ACBF] text-[10px] uppercase tracking-[3px] mb-1">Reference Code</p>
                        <p className="font-mono font-bold text-[#A7EBF2] text-xl tracking-[4px]">{refCode}</p>
                      </div>

                      <div className="bg-[rgba(1,28,64,0.4)] border border-[rgba(84,172,191,0.08)] rounded-xl p-4 text-left space-y-2">
                        <p className="font-mono text-[#54ACBF] text-[10px] uppercase tracking-[2px] mb-2">What's Next</p>
                        {[
                          ...
                          <div key={i} className="flex items-center gap-2.5 font-sans text-[#8BBFCC] text-xs">
                            <span className="text-[#54ACBF] flex-shrink-0">{item.icon}</span>{item.text}
                          </div>
                        ))}
                      </div>
                ...
                {/* Footer buttons */}
                {step < 4 && (
                  <div className="flex-shrink-0 px-6 pb-5 pt-3 border-t border-[rgba(84,172,191,0.08)] flex gap-3">
                    {step > 0 && (
