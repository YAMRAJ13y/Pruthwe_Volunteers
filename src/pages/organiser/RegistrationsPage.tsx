...
function VolunteerRow({
  ...
  return (
    <>
      <tr className={`border-b border-[rgba(84,172,191,0.07)] transition-colors ${selected ? 'bg-[rgba(84,172,191,0.06)]' : 'hover:bg-[rgba(84,172,191,0.03)]'}`}>
        <td className="py-3 pl-4 pr-2">
          <input type="checkbox" checked={selected} onChange={onToggle}
            className="w-4 h-4 rounded accent-[#54ACBF] cursor-pointer" />
        </td>
        <td className="py-3 px-3">
          <div className="flex items-center gap-3">
            <div className="w-8 h-8 rounded-xl flex items-center justify-center font-display font-black text-xs text-[#011C40] flex-shrink-0"
              style={{ background: reg.tierColor + '99' }}>
              {reg.name.split(' ').map(p => p[0]).join('').slice(0,2)}
            </div>
            <div>
              <p className="font-display font-bold text-[#F0FAFB] text-sm">{reg.name}</p>
              <p className="font-sans text-[#8BBFCC] text-xs flex items-center gap-1">
        ...
        <td className="py-3 px-3 hidden md:table-cell">
          <span className="font-sans text-[#8BBFCC] text-xs flex items-center gap-1">
            <Mail size={10} />{reg.email}
          </span>
        </td>
        <td className="py-3 px-3">
          <div className="flex items-center gap-2">
            <div className="flex-1 h-1.5 bg-[rgba(84,172,191,0.12)] rounded-full max-w-[48px]">
        ...
        <td className="py-3 pl-2 pr-4">
          <div className="flex items-center gap-1.5">
            ...
            <button onClick={() => setExpanded(e => !e)}
              className="p-1.5 rounded-lg border border-[rgba(84,172,191,0.12)] text-[#8BBFCC] hover:text-[#F0FAFB] transition-colors">
          ...
          <tr>
            <td colSpan={7} className="p-0">
              <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }}
                ...
                className="overflow-hidden bg-[rgba(84,172,191,0.04)] border-b border-[rgba(84,172,191,0.08)]">
                <div className="px-6 py-4 grid grid-cols-2 md:grid-cols-4 gap-4">
                  <div><p className="font-mono text-[#54ACBF] text-[10px] uppercase tracking-wide mb-1">Email</p><p className="font-sans text-[#F0FAFB] text-sm">{reg.email}</p></div>
                  <div><p className="font-mono text-[#54ACBF] text-[10px] uppercase tracking-wide mb-1">City</p><p className="font-sans text-[#F0FAFB] text-sm">{reg.city}</p></div>
                  <div><p className="font-mono text-[#54ACBF] text-[10px] uppercase tracking-wide mb-1">Tier</p><p className="font-display font-bold text-sm" style={{ color: reg.tierColor }}>{reg.tier}</p></div>
                  <div><p className="font-mono text-[#54ACBF] text-[10px] uppercase tracking-wide mb-1">MQ Score</p><p className="font-mono text-sm" style={{ color: mqColor(reg.mqScore) }}>{reg.mqScore} / 100</p></div>
                </div>
              </motion.div>
            </td>
          </tr>
      ...
      <div className="max-w-7xl mx-auto px-5 md:px-8 py-8 space-y-6">
        ...
        <div className="grid grid-cols-3 gap-4">
          {[
            ...
            <div key={s.label} className="bg-[#023859] border border-[rgba(84,172,191,0.1)] rounded-2xl p-4 text-center">
              <p className="font-display font-black leading-none mb-1" style={{ fontSize: 'clamp(24px,3vw,36px)', color: s.color }}>{s.value}</p>
              <p className="font-mono text-[#8BBFCC] text-[10px] uppercase tracking-[2px]">{s.label}</p>
            </div>
          ))}
        </div>
        ...
        <div className="flex flex-col sm:flex-row gap-3 items-start sm:items-center justify-between">
          ...
          <div className="flex gap-2 w-full sm:w-auto">
            ...
            <button className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl border border-[rgba(84,172,191,0.15)] text-[#8BBFCC] hover:text-[#F0FAFB] transition-colors font-display font-bold text-[10px] uppercase tracking-wide">
        ...
        <AnimatePresence>
          {selected.size > 0 && (
            <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }}
              className="flex items-center gap-3 px-4 py-3 bg-[rgba(84,172,191,0.08)] border border-[rgba(84,172,191,0.2)] rounded-xl">
              <span className="font-display font-bold text-[#A7EBF2] text-xs uppercase tracking-wide">{selected.size} selected</span>
              ...
              <button onClick={() => setSelected(new Set())} className="ml-auto text-[#8BBFCC] hover:text-[#F0FAFB] transition-colors font-sans text-xs">Clear</button>
            </motion.div>
          )}
        </AnimatePresence>
        ...
        <div className="bg-[#023859] border border-[rgba(84,172,191,0.1)] rounded-2xl overflow-hidden">
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead>
                <tr className="border-b border-[rgba(84,172,191,0.1)]">
                  <th className="py-3 pl-4 pr-2 w-8">
                    <input type="checkbox" checked={allSelected} onChange={toggleAll}
                      className="w-4 h-4 rounded accent-[#54ACBF] cursor-pointer" />
                  </th>
                  {['Volunteer', 'Email', 'MQ Score', 'Tier', 'Status', 'Actions'].map(h => (
                    <th key={h} className={`py-3 px-3 text-left font-mono text-[#54ACBF] text-[10px] uppercase tracking-[2px] ${h === 'Email' ? 'hidden md:table-cell' : h === 'Tier' ? 'hidden sm:table-cell' : ''}`}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {filtered.length === 0 ? (
                  <tr><td colSpan={7} className="py-16 text-center">
                    <Users size={28} className="text-[#54ACBF] mx-auto mb-2 opacity-40" />
                    <p className="font-display font-bold text-[#8BBFCC] text-sm uppercase tracking-wide">No registrations found</p>
                  </td></tr>
                ) : filtered.map(reg => (
          ...
          <div className="px-4 py-3 border-t border-[rgba(84,172,191,0.07)] flex items-center justify-between">
            <p className="font-sans text-[#8BBFCC] text-xs">Showing {filtered.length} of {regs.length} registrations</p>
            <div className="flex items-center gap-1">
              <Filter size={11} className="text-[#54ACBF]" />
              <span className="font-mono text-[#54ACBF] text-[10px] uppercase tracking-wide">Sorted by MQ Score</span>
            </div>
          </div>
        </div>
      </div>
